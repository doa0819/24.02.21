...
public class Application {
    public static void main(String[] args) {
        ...
        *   클래스 또한 부모 클래스와 자식 클래스로 역할을 나누어서 부모가 가지는 맴버를 자식이
        *   물려 받아 자기의 맴버인 것 처럼 사용할 수 있도록 만든 기술이다.
        *
        * 필기.
        *  하지만 단순 물려받는 개념보다 조금 더 나아간다면
        *   자바에서의 상속은 부모클래스의 확장(extend)의 개념을 가진다.
        *   물려 받아서 자신의 것 처럼 사용하는 것 뿐 아니라 추가적인 맴버도 작성이 가능하다.
        ...
        *   2. 클래스간의 계층 관계가 형성되면 다형성의 문법적인 토대가 된다.
        *
        * 필기.
        *  하지만 상속으로 인한 단점도 존재한다.
        *   1. 부모클래스의 가눙울 추가/변경 할 시 자식클래스가 정상적으로 동작하는 지 예측이 힘들다.
        ...
        * */
        ...
        // 생산성 -> 코드를 덜 친다, 이미 만들어 놨던 거를 재사용하는 것이니 생산성이 높아진다.
        // 맴버 -> 메소드,
        // 다형성 ->
        ...
        /* 목차. 2. FireCar 인스턴스 생성 후 Car 클래스에 작성항 메소드 사용 가능 확인. */
        FireCar fireCar = new FireCar();

        /* 필기. 우리는 분명 아무것도 작성하지 않았는데 Car(부모)클래스가 가진 메소드를 사용했다. */
        fireCar.soundHorn();             // 생산성이 높아졌다 -> 작성하지 않은 메소드를 부모클래스에서 가져올수 있어서
        fireCar.run();
        fireCar.soundHorn();
        fireCar.stop();
        fireCar.soundHorn();
